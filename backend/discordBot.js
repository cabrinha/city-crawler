...
import { parseCredits, parseGuildLocations, parseShopLocations } from './parseMessages.js';
...
async function processGuildMessageById(messageId, channelId, interactionUser) {
  try {
    ...
    const guilds = parseGuildLocations(message.content, { warn: logWarning });
...
async function processMessageById(messageId, channelId, interactionUser) {
  try {
    ...
    const shops = parseShopLocations(message.content, { warn: logWarning });
    const creditedUsers = parseCredits(message.content);
