...
func (User) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Time("created_at").
			Default(time.Now).
			Immutable(),
		field.Int("total_reports").
			Default(0).
			Comment("Total number of reports submitted by this user"),
	}
}
...
func (User) Edges() []ent.Edge {
	return []ent.Edge{
		edge.From("owner", PasswordToken.Type).
			Ref("user"),
		edge.From("location_reports", LocationReport.Type).
			Ref("reporter"),
		edge.From("blood_deity_reports", BloodDeity.Type).
			Ref("reporter"),
		edge.From("rich_vampire_reports", RichVampire.Type).
			Ref("reporter"),
	}
}
